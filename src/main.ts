...
import { MASTER_CHART_DATA, getDataTimeRange } from "./chart/hardcodedData";
...
// Get data range from master data
const { minPrice, maxPrice } = getDataTimeRange(MASTER_CHART_DATA);
const priceMargin = (maxPrice - minPrice) * 0.1;

// Initialize with master data
state.chartData = MASTER_CHART_DATA;
...
function setTimeframe(tf: Timeframe) {
  ...
  const lastTime = MASTER_CHART_DATA[MASTER_CHART_DATA.length - 1].epoch * 1000;
  const windowMs = tfConfig.defaultRange;
  const rightPadMs = windowMs * 0.1;
  
  state.timeStart = lastTime - windowMs;
  state.timeEnd = lastTime + rightPadMs;
  state.timeZoomLevel = 0;
  state.priceZoomLevel = 0;
...
timeframeControls.style.cssText = "display:flex;gap:12px;padding:12px 20px;background:#f1f5f9;border-top:1px solid #e2e8f0;justify-content:center;flex-wrap:wrap;";
...
setupChartEvents({ canvas, container, ctx, dpr: DPR, state, redraw: draw });
